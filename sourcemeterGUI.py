...
for instrument in rm.list_resources():
    ...
    try:
        if instr.query("*IDN?")[:36] == 'KEITHLEY INSTRUMENTS INC.,MODEL 2400':
            break
    except:
        pass
...
class IVProcedure(Procedure):
    ...
    filename = Parameter('Filename', default=unique_filename(str(directory), prefix='SourceI_'))
...
class MainWindow(ManagedWindow):
    ...
    def __init__(self):
        super(MainWindow, self).__init__(
            ...
            inputs=[
                'current', 'delay', 'voltage_range', 'filename'
            ],
            displays=[
                'current', 'delay', 'voltage_range'
            ],
            x_axis='Current (A)',
            y_axis='Voltage (V)'
        )
    ...
    def queue(self):
        filename = str(IVProcedure.filename)
